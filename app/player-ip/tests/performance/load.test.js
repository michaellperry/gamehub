...
class PerformanceTestRunner {
  ...
  async startTestServer() {
    console.log(`${colors.blue}🔧 Starting test server for performance tests...${colors.reset}`);
  ...
  async testHealthCheckPerformance() {
    await this.runTest('Health Check Performance', async () => {
      ...
      console.log(`${colors.blue}     Average: ${avgResponseTime.toFixed(2)}ms${colors.reset}`);
      console.log(`${colors.blue}     Min: ${minResponseTime}ms, Max: ${maxResponseTime}ms${colors.reset}`);
      
      assert.ok(avgResponseTime < TEST_CONFIG.performanceThresholds.healthCheckMaxMs, 
      ...
      console.log(`${colors.green}  ✓ Health check performance within acceptable limits${colors.reset}`);
    });
  }
  ...
  async testTokenGenerationPerformance() {
    await this.runTest('Token Generation Performance', async () => {
      ...
      console.log(`${colors.blue}     Average: ${avgGenerationTime.toFixed(3)}ms${colors.reset}`);
      console.log(`${colors.blue}     Min: ${minGenerationTime.toFixed(3)}ms, Max: ${maxGenerationTime.toFixed(3)}ms${colors.reset}`);
      
      assert.ok(avgGenerationTime < TEST_CONFIG.performanceThresholds.tokenGenerationMaxMs, 
      ...
      console.log(`${colors.green}  ✓ Token generation performance within acceptable limits${colors.reset}`);
    });
  }
  ...
  async testDatabasePerformance() {
    await this.runTest('Database Performance', async () => {
      ...
      console.log(`${colors.blue}     Average: ${avgOperationTime.toFixed(3)}ms${colors.reset}`);
      console.log(`${colors.blue}     Min: ${minOperationTime.toFixed(3)}ms, Max: ${maxOperationTime.toFixed(3)}ms${colors.reset}`);
      
      assert.ok(avgOperationTime < TEST_CONFIG.performanceThresholds.databaseOperationMaxMs, 
      ...
      console.log(`${colors.green}  ✓ Database performance within acceptable limits${colors.reset}`);
    });
  }
  ...
  async testConcurrentHealthChecks() {
    await this.runTest('Concurrent Health Checks', async () => {
      ...
      console.log(`${colors.blue}     Successful requests: ${successfulRequests}/${concurrentRequests}${colors.reset}`);
      console.log(`${colors.blue}     Average response time: ${avgResponseTime.toFixed(2)}ms${colors.reset}`);
      console.log(`${colors.blue}     Max response time: ${maxResponseTime}ms${colors.reset}`);
      
      assert.strictEqual(successfulRequests, concurrentRequests, 'All concurrent requests should succeed');
      assert.ok(maxResponseTime < TEST_CONFIG.performanceThresholds.concurrentRequestMaxMs, 
        `Max response time ${maxResponseTime}ms exceeds threshold ${TEST_CONFIG.performanceThresholds.concurrentRequestMaxMs}ms`);
      
      console.log(`${colors.green}  ✓ Concurrent health checks handled successfully${colors.reset}`);
    });
  }
  ...
  async testConcurrentAuthentication() {
    await this.runTest('Concurrent Authentication Requests', async () => {
      ...
      assert.strictEqual(successfulRequests, concurrentRequests, 'All concurrent authentication requests should be handled');
      
      console.log(`${colors.green}  ✓ Concurrent authentication requests handled successfully${colors.reset}`);
    });
  }
  ...
  async testSustainedLoad() {
    await this.runTest('Sustained Load Test', async () => {
      ...
      console.log(`${colors.blue}  🔄 Running sustained load for ${duration/1000} seconds...${colors.reset}`);
      ...
      console.log(`${colors.blue}     Success rate: ${successRate.toFixed(2)}%${colors.reset}`);
      console.log(`${colors.blue}     Average response time: ${avgResponseTime.toFixed(2)}ms${colors.reset}`);
      
      assert.ok(successRate > 95, `Success rate ${successRate.toFixed(2)}% is below 95%`);
      
      console.log(`${colors.green}  ✓ Sustained load test completed successfully${colors.reset}`);
    });
  }
  ...
  async testMemoryUsage() {
    await this.runTest('Memory Usage Under Load', async () => {
      ...
      const { createUser, storeUserIdentity } = await import('../../dist/repository/index.js');
      ...
      console.log(`${colors.blue}     Initial heap: ${(initialMemory.heapUsed / 1024 / 1024).toFixed(2)}MB${colors.reset}`);
      console.log(`${colors.blue}     Final heap: ${(finalMemory.heapUsed / 1024 / 1024).toFixed(2)}MB${colors.reset}`);
      console.log(`${colors.blue}     Memory increase: ${memoryIncreaseMB.toFixed(2)}MB${colors.reset}`);
      
      assert.ok(memoryIncreaseMB < TEST_CONFIG.performanceThresholds.memoryLeakThresholdMB, 
  ...
  async testErrorRateUnderLoad() {
    await this.runTest('Error Rate Under Load', async () => {
      ...
      console.log(`${colors.green}  ✓ Error rate under load within acceptable limits${colors.reset}`);
    });
  }
  ...
  printResults() {
    ...
    if (this.performanceMetrics.responseTimes.length > 0) {
      ...
      this.performanceMetrics.responseTimes.forEach(metric => {
        console.log(`${colors.magenta}${metric.test}: avg ${metric.avg.toFixed(3)}ms (${metric.min.toFixed(3)}-${metric.max.toFixed(3)}ms)${colors.reset}`);
      });
    }
