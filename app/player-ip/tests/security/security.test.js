...
class SecurityTestRunner {
  ...
  async testSQLInjectionPrevention() {
    await this.runTest('SQL Injection Prevention', async () => {
      ...
      for (const payload of sqlInjectionPayloads) {
        ...
        assert.notStrictEqual(tokenResponse.status, 500, 
    ...
    await this.runTest('XSS Prevention', async () => {
      ...
      for (const payload of xssPayloads) {
        ...
        if (response.headers.get('content-type')?.includes('text/html')) {
          ...
          assert.ok(!htmlContent.includes('javascript:'), 
            `JavaScript protocol in payload "${payload}" not properly handled`);
          assert.ok(!htmlContent.includes('onerror='), 
  ...
  async testAuthenticationBypass() {
    await this.runTest('Authentication Bypass Prevention', async () => {
      ...
      console.log(`${colors.green}  ✓ Authentication bypass prevention verified${colors.reset}`);
    });
  }
  ...
  async testParameterPollution() {
    await this.runTest('Parameter Pollution Prevention', async () => {
      ...
      for (const params of pollutedParams) {
        const response = await fetch(`${TEST_CONFIG.baseUrl}/authenticate?${params}&response_type=code&code_challenge_method=S256`);
      ...
      console.log(`${colors.green}  ✓ Parameter pollution prevention verified${colors.reset}`);
    });
  }
  ...
  async testErrorInformationDisclosure() {
    await this.runTest('Error Information Disclosure Prevention', async () => {
      ...
      for (const pattern of sensitivePatterns) {
        assert.ok(!pattern.test(errorMessage), 
          `Error message should not contain sensitive information: ${errorMessage}`);
      }
      
      console.log(`${colors.green}  ✓ Error information disclosure prevention verified${colors.reset}`);
    });
  }
  ...
  async testPKCESecurity() {
    await this.runTest('PKCE Security', async () => {
      ...
      assert.strictEqual(verifyCodeChallenge(codeVerifier, codeChallenge, 'S256'), true,
      ...
      assert.strictEqual(verifyCodeChallenge(wrongVerifier, codeChallenge, 'S256'), false,
        'Invalid PKCE verifier should fail');
      
      // Test that plain method is supported but less secure
      assert.strictEqual(verifyCodeChallenge('test-verifier', 'test-verifier', 'plain'), true,
  ...
  async testStateParameterValidation() {
    await this.runTest('State Parameter Validation', async () => {
      ...
      assert.ok(response.status === 400, 'Should handle authentication request with state parameter');
  ...
  printResults() {
    ...
    if (this.securityFindings.length > 0) {
      ...
      this.securityFindings.forEach((finding, index) => {
        ...
        console.log(`${index + 1}. ${severityColor}[${finding.severity}]${colors.reset} ${finding.finding}`);
    ...
    if (failed > 0) {
      ...
      if (this.securityFindings.length > 0) {
        console.log(`${colors.yellow}⚠️  ${this.securityFindings.length} security finding(s) noted for review.${colors.reset}`);
      }
    }
  }
}
