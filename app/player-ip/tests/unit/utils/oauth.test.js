...
describe('OAuth Utilities', () => {
  describe('verifyCodeChallenge', () => {
    ...
    test('should handle method case sensitivity', () => {
      ...
      const s256Challenge = crypto.createHash('sha256').update(codeVerifier).digest('base64url');
  ...
  describe('PKCE Flow Integration', () => {
    ...
    test('should reject tampered code verifier', () => {
      ...
      const codeChallenge = crypto.createHash('sha256').update(originalVerifier).digest('base64url');
    ...
    test('should reject tampered code challenge', () => {
      const codeVerifier = crypto.randomBytes(32).toString('base64url');
      const originalChallenge = crypto.createHash('sha256').update(codeVerifier).digest('base64url');
