...
class APIContractTestRunner {
  ...
  async testHealthCheckContract() {
    await this.runTest('Health Check Endpoint Contract', async () => {
      ...
      console.log(`${colors.green}  ✓ Health check endpoint contract verified${colors.reset}`);
    });
  }
  ...
  async testPublicKeyEndpointContract() {
    await this.runTest('Public Key Endpoint Contract', async () => {
      ...
      // Content-Type contract
      const contentType = response.headers.get('content-type');
      assert.ok(contentType.includes('application/json'), 'Public key endpoint should return JSON');
      ...
      assert.ok(data.publicKey.startsWith('-----BEGIN'), 'Public key should be in PEM format');
      assert.ok(data.publicKey.includes('-----END'), 'Public key should be complete PEM format');
  ...
  async testAuthenticateEndpointContract() {
    await this.runTest('Authenticate Endpoint Contract', async () => {
      ...
      assert.ok(htmlContent.includes('QR Code Required'), 'Should show QR code required message');
      assert.ok(htmlContent.includes('<!DOCTYPE html>'), 'Should be valid HTML');
      
      console.log(`${colors.green}  ✓ Authenticate endpoint contract verified${colors.reset}`);
    });
  }
  ...
  async testTokenEndpointContract() {
    await this.runTest('Token Endpoint Contract', async () => {
      ...
      // Content-Type contract
      const contentType = response.headers.get('content-type');
      assert.ok(contentType.includes('application/json'), 'Token error response should be JSON');
      ...
      assert.strictEqual(unsupportedResponse.status, 400, 'Should return 400 for unsupported grant');
      
      const unsupportedError = await unsupportedResponse.json();
      assert.ok(unsupportedError.error.includes('Unsupported grant_type'), 'Should specify unsupported grant type');
      ...
      assert.strictEqual(authCodeResponse.status, 400, 'Should return 400 for missing auth code params');
      
      const authCodeError = await authCodeResponse.json();
      assert.ok(authCodeError.error.includes('Missing required parameters'), 'Should specify missing parameters');
  ...
  async testErrorResponseContract() {
    await this.runTest('Error Response Contract', async () => {
      ...
      assert.strictEqual(notFoundResponse.status, 404, 'Should return 404 for non-existent endpoints');
      ...
      assert.ok(malformedResponse.status >= 400, 'Should return error status for malformed JSON');
      ...
      if (malformedResponse.headers.get('content-type')?.includes('application/json')) {
        ...
        assert.strictEqual(typeof errorData, 'object', 'JSON error responses should be objects');
        assert.ok(errorData.error || errorData.message, 'Should include error or message field');
      }
