...
class TestRunner {
  ...
  async setupTestEnvironment() {
    ...
    process.env.TENANT_PUBLIC_KEY = '-----BEGIN PUBLIC KEY-----\nMIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA1234567890\n-----END PUBLIC KEY-----';
  ...
  async testHealthCheckEndpoint() {
    await this.runTest('Health Check Endpoint', async () => {
      ...
      console.log(`${colors.green}  ✓ Health check endpoint working correctly${colors.reset}`);
    });
  }
  ...
  async testOAuth2Endpoints() {
    await this.runTest('OAuth 2.0 Endpoints', async () => {
      ...
      const authResponseMissingGap = await fetch(`${TEST_CONFIG.baseUrl}/authenticate?${params}`);
      
      if (authResponseMissingGap.status === 400) {
      ...
      console.log(`${colors.green}  ✓ OAuth 2.0 endpoints properly validate parameters${colors.reset}`);
    });
  }
  ...
  async testGAPIntegration() {
    await this.runTest('GAP Integration (Basic)', async () => {
      ...
      console.log(`${colors.green}  ✓ GAP integration basic functionality working${colors.reset}`);
    });
  }
