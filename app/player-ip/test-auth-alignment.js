...
import { getServiceToken, makeAuthenticatedRequest } from './src/utils/service-client.js';
...
async function testServiceToServiceAuth() {
    ...
    try {
        ...
        try {
            ...
            logTest('Service Token Acquisition', 'PASSED', 'Service token acquired successfully');
        } catch (error) {
            // Expected if service-ip is not running
            if (
                error.message.includes('Failed to get service token') ||
...
async function testJWTTokenClaims() {
    ...
    try {
        ...
        if (decoded.iss !== TEST_CONFIG.EXPECTED_JWT_CLAIMS.iss) {
            throw new Error(
                `Invalid issuer: expected ${TEST_CONFIG.EXPECTED_JWT_CLAIMS.iss}, got ${decoded.iss}`
            );
        }

        if (decoded.aud !== TEST_CONFIG.EXPECTED_JWT_CLAIMS.aud) {
            throw new Error(
                `Invalid audience: expected ${TEST_CONFIG.EXPECTED_JWT_CLAIMS.aud}, got ${decoded.aud}`
            );
        }
...
async function testGameHubModelIntegration() {
    ...
    try {
        ...
        logTest('User Fact Creation', 'PASSED', `User created with ID: ${testUser.id}`);
...
async function testGAPAuthorization() {
    ...
    try {
        ...
        const isAuthorized =
            retrievedGAP.type === GAPType.OPEN &&
        ...
        logTest('GAP Authorization Check', 'PASSED', 'GAP authorization logic working correctly');
    } catch (error) {
        logTest('GAP Authorization', 'FAILED', 'GAP authorization test failed', error);
    }
}
...
async function testFrontendIntegration() {
    ...
    try {
        ...
        logTest('CORS Configuration', 'PASSED', `CORS origin configured: ${corsOrigin}`);
        ...
        logTest('Frontend Integration', 'FAILED', 'Frontend integration test failed', error);
    }
}
...
async function testCrossServiceCommunication() {
    ...
    try {
        ...
        try {
            ...
            throw new Error(
                `Service authentication provider import failed: ${importError.message}`
            );
        }
    } catch (error) {
        logTest(
...
async function testTokenRefreshFlow() {
    ...
    try {
        ...
        logTest('Token Refresh Flow', 'FAILED', 'Token refresh flow test failed', error);
    }
}
...
async function testErrorHandling() {
    ...
    try {
        ...
        logTest('Invalid JWT Token Handling', 'PASSED', 'Invalid JWT tokens properly rejected');
        ...
        logTest('Invalid PKCE Handling', 'PASSED', 'Invalid PKCE challenges properly rejected');
        ...
        logTest('Invalid Cookie Handling', 'PASSED', 'Invalid cookies properly handled');
    } catch (error) {
        logTest('Error Handling', 'FAILED', 'Error handling test failed', error);
    }
}
...
async function testSecurityValidation() {
    ...
    try {
        ...
        logTest('Cookie Security', 'PASSED', 'Cookie values are sufficiently random and secure');

        // Test authorization code security
        const authCode1 = createAuthorizationCode(
            'client1',
            'uri1',
            'challenge1',
            'S256',
        ...
        const authCode2 = createAuthorizationCode(
            'client2',
            'uri2',
            'challenge2',
            'S256',
        ...
        logTest('Security Validation', 'FAILED', 'Security validation test failed', error);
    }
}
