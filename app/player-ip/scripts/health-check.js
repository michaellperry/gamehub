...
class HealthChecker {
    ...
    async checkHealthEndpoint() {
        await this.runCheck('Health Endpoint', async () => {
            ...
            console.log(
                `${colors.green}  ✓ Health endpoint responding correctly (${responseTime}ms)${colors.reset}`
            );
        });
    }
    ...
    async checkResponseTimes() {
        await this.runCheck('Response Times', async () => {
            ...
            if (avgResponseTime > CONFIG.thresholds.responseTimeMs) {
                throw new Error(
                    `Average response time ${avgResponseTime.toFixed(2)}ms exceeds threshold ${CONFIG.thresholds.responseTimeMs}ms`
                );
            }

            console.log(
                `${colors.green}  ✓ Response times acceptable (avg: ${avgResponseTime.toFixed(2)}ms, max: ${maxResponseTime}ms)${colors.reset}`
            );
        });
    }
    ...
    async checkResourceUsage() {
        await this.runCheck('Resource Usage', async () => {
            ...
            if (memoryUsageMB > CONFIG.thresholds.memoryUsageMB) {
                console.log(
                    `${colors.yellow}  ⚠️  Memory usage ${memoryUsageMB.toFixed(2)}MB exceeds threshold ${CONFIG.thresholds.memoryUsageMB}MB${colors.reset}`
                );
            } else {
                console.log(
                    `${colors.green}  ✓ Memory usage acceptable (${memoryUsageMB.toFixed(2)}MB)${colors.reset}`
                );
            }

            // Note: CPU usage would require additional monitoring tools in production
            console.log(
                `${colors.blue}  ℹ️  For CPU monitoring, integrate with system monitoring tools${colors.reset}`
            );
        });
    }
        ...
        for (let attempt = 1; attempt <= CONFIG.retries; attempt++) {
            try {
                ...
                if (attempt < CONFIG.retries) {
                    console.log(
                        `${colors.yellow}  ⚠️  Attempt ${attempt} failed, retrying...${colors.reset}`
                    );
