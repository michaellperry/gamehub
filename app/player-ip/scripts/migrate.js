...
class DatabaseMigrator {
    ...
    async loadMigrations() {
        ...
        if (!fs.existsSync(CONFIG.migrationsDir)) {
            console.log(
                `${colors.blue}  No migrations directory found, creating...${colors.reset}`
            );
            return;
        }
        ...
        console.log(
            `${colors.green}  ✓ Loaded ${this.migrations.length} migration files${colors.reset}`
        );
    }
...
`.trim();
            ...
            console.log(
                `${colors.green}  ✓ Created initial migration: ${migration001Path}${colors.reset}`
            );
        }
    }
    ...
    async getCurrentVersion() {
      ...
      `
            ...
            console.log(
                `${colors.blue}Current database version: ${this.currentVersion}${colors.reset}`
            );
        } catch (error) {
            this.currentVersion = 0;
            console.log(
                `${colors.blue}No migration history found, starting from version 0${colors.reset}`
            );
        }
    }
    ...
    async runMigrations() {
        ...
        if (pendingMigrations.length === 0) {
            console.log(
                `${colors.green}✅ Database is up to date (version ${this.currentVersion})${colors.reset}`
            );
            return;
        }

        console.log(
            `${colors.yellow}🔄 Running ${pendingMigrations.length} pending migrations...${colors.reset}`
        );
    ...
    async runMigration(migration) {
        console.log(
            `${colors.cyan}📝 Applying migration ${migration.version}: ${migration.name}${colors.reset}`
        );
      ...
      `
            ...
            if (existing) {
                if (existing.checksum !== migration.checksum) {
                    throw new Error(
                        `Migration ${migration.version} checksum mismatch. Database may have been modified.`
                    );
                }
                console.log(
                    `${colors.yellow}  ⚠️  Migration ${migration.version} already applied, skipping${colors.reset}`
                );
                return;
            }
        ...
        `
            ...
            console.log(
                `${colors.green}  ✅ Migration ${migration.version} applied successfully${colors.reset}`
            );
        } catch (error) {
    ...
    async rollbackMigrations() {
        ...
        if (targetVersion >= this.currentVersion) {
            console.log(
                `${colors.yellow}⚠️  No rollback needed. Current version: ${this.currentVersion}, Target: ${targetVersion}${colors.reset}`
            );
            return;
        }

        console.log(
            `${colors.yellow}🔄 Rolling back to version ${targetVersion}...${colors.reset}`
        );

        if (!CONFIG.force) {
            console.log(
                `${colors.red}⚠️  Rollback is destructive and may cause data loss!${colors.reset}`
            );
            console.log(
                `${colors.red}Use --force flag to confirm rollback operation.${colors.reset}`
            );
            return;
        }
        ...
        console.log(
            `${colors.red}⚠️  SQLite rollback requires database recreation. This will delete all data!${colors.reset}`
        );

        if (CONFIG.dryRun) {
            console.log(
                `${colors.blue}[DRY RUN] Would recreate database and apply migrations up to version ${targetVersion}${colors.reset}`
            );
        }
    }
    ...
    async verifyIntegrity() {
        ...
        try {
            ...
            if (foreignKeyResult.length > 0) {
                throw new Error(
                    `Foreign key constraint violations found: ${JSON.stringify(foreignKeyResult)}`
                );
            }
