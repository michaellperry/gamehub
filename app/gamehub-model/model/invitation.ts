...
export class Invitation {
    ...
    static in(tenant: LabelOf<Tenant>) {
        return tenant.successors(Invitation, invitation => invitation.tenant as LabelOf<Tenant>)
            .notExists(invitation => invitation.successors(InvitationClosure, closure => closure.invitation as LabelOf<Invitation>));
    }
}
...
export class InvitationAcceptance {
    ...
    static for(invitation: LabelOf<Invitation>) {
        return invitation.successors(InvitationAcceptance, acceptance => acceptance.invitation)
            .notExists(acceptance => acceptance.successors(InvitationApproval, approval => approval.acceptance))
            .notExists(acceptance => acceptance.successors(InvitationDecline, decline => decline.acceptance));
    }
}
...
export class InvitationApproval {
    ...
    constructor(
        public acceptance: InvitationAcceptance
    ) { }
...
export const invitationModel = (b: ModelBuilder) => b
    .type(Invitation, m => m
        .predecessor("tenant", Tenant)
    )
    .type(InvitationAcceptance, m => m
        .predecessor("invitation", Invitation)
        .predecessor("user", User)
    )
    .type(InvitationApproval, m => m
        .predecessor("acceptance", InvitationAcceptance)
    )
    .type(InvitationDecline, m => m
        .predecessor("acceptance", InvitationAcceptance)
    )
    .type(InvitationClosure, m => m
        .predecessor("invitation", Invitation)
    );
