...
export class ServicePrincipal {
    ...
    static usersOf(tenant: LabelOf<Tenant>) {
        return ServicePrincipal.of(tenant).selectMany((servicePrincipal) =>
            servicePrincipal.user.predecessor()
        );
    }
}

export const bookkeepingModel = (b: ModelBuilder) =>
    b.type(ServicePrincipal, (x) => x.predecessor('tenant', Tenant).predecessor('user', User));
